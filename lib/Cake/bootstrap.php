...
function getMethodOffObject($oObject, $sMethodName){ // getMethodOffObject($oCat, 'sayMeow') == $oCat->sayMeow
    $fResult = new ReflectionMethod($oObject, $sMethodName);
    return $fResult->getClosure($oObject);
} ;

function expect( $array, $sKey, $sError = "Assertion failed"){
    if (! array_key_exists($sKey, $array)) {
        throw new Exception($sError);
    }
    return $array[$sKey];
}

function ifset ($array, $sKey, $vDefault){
    return array_key_exists($sKey, $array) ? $array[$sKey] : $vDefault;
}
